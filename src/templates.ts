...
export function getDefaultRalphLoopModuleReadme(): string {
  return [
    ...
    '',
    'Ralph Loop is a generic artifact refinement workflow. The host AI agent (Claude Code or Codex CLI) acts as Orchestrator, coordinating Planner, Generator, Evaluator, and Assembler roles in a scored loop.',
    '',
    'The document workflow is the first adapter, not the boundary of the system. Code implementation, refactor, design-to-code, and decision/analysis tasks use the same loop with adapter-specific rubrics and safety rules.',
    '',
    '## Key files',
    '- `skill.md` — Orchestrator behavior: trigger, self-check, adapter selection, decomposition judgment, loop control',
    '- `prompts/` — Generator, Evaluator, and Assembler subagent payload templates',
    '- `rubrics/` — Built-in rubrics (`one-pager.yaml`, `code-implementation.yaml`, `assembly.yaml`) and authoring guide',
    '- `references/` — Artifact adapters, host-profile model-tier mapping, decomposition heuristics, iteration-log schema',
...
export function getDefaultRalphLoopSkill(): string {
  return [
    ...
    '## Trigger',
    '',
    'Use this skill when the user asks to generate, refine, or improve a structured artifact through iterative Planner, Generator, Evaluator, and Assembler collaboration.',
    '',
    'Examples include:',
    '- documents: one-pagers, PRDs, RFCs, briefs, long-form docs',
    '- code implementation, refactor, design-to-code, or implementation planning',
    '- decision or analysis tasks that benefit from explicit scoring and revision',
    '',
    'Ralph Loop is a generic artifact refinement workflow. Do not reject code or decision tasks just because the original v1 examples were documents.',
    '',
    ...
    'Selected adapter:  <document | code | decision | user-defined>',
    'Execution mode:    <document | plan/patch mode | direct implementation mode | decision>',
    'Rubric:            <path>',
    ...
    '',
    'Planned action:',
    '  1. Classify task, choose adapter, execution mode, and rubric',
    '  2. Collect or create task-spec and Planner output',
    '  3. Score task for decomposition',
    '  4. Run Generator → Evaluator loop (up to max_rounds rounds)',
    '  5. Emit adapter-specific best artifact + summary.json',
    '',
    'Awaiting user confirmation…',
    '```',
    '',
    'For code tasks, plan/patch mode is the default. direct implementation mode is allowed only when the user explicitly permits Ralph Loop subagents to edit code.',
    '',
    ...
    '',
    '## Step 1 — Classify task and choose adapter',
    '',
    'Read `references/artifact-adapters.md` before choosing an adapter.',
    '',
    '- Choose `document` for one-pagers, PRDs, RFCs, briefs, or other prose artifacts.',
    '- Choose `code` for code implementation, refactor, design-to-code, or implementation planning.',
    '- Choose `decision` for technical choices, architecture tradeoffs, or recommendation memos.',
    '- If no adapter fits, ask the user for a rubric or define a user-specific adapter in the run inputs.',
    '',
    'Default rubrics:',
    '- document: `rubrics/one-pager.yaml` unless the user provides another rubric',
    '- code: `rubrics/code-implementation.yaml`',
    '- decision: user-provided rubric in v1',
    '',
    'For code tasks:',
    '- default to plan/patch mode, producing an implementation plan, patch proposal or diff guidance, test strategy, risk notes, and blockers',
    '- use direct implementation mode only with explicit user permission',
    '- in direct implementation mode, assign disjoint file/module ownership to workers and never allow workers to revert unrelated edits',
    '',
    '## Step 2 — Collect inputs and Planner output',
    '',
    '**Fast path (interactive):** Ask the user:',
    '1. "What artifact are we improving or producing? (one sentence)"',
    '2. "Which adapter/rubric? (press Enter for the default if available)"',
    '3. "Any explicit constraints? (scope, files, audience, verification, risk)"',
    '',
    ...
    '',
    'Planner role:',
    '- For simple tasks, create the Planner output in-session and save it with the task spec.',
    '- For complex tasks, dispatch a Planner subagent to produce the implementation plan, decomposition proposal, constraints, and success criteria.',
    '',
    'Load and validate the rubric YAML (see Rubric Validation below).',
    '',
    '## Step 3 — Decomposition judgment',
    '',
    ...
    '',
    '- **no split:** proceed to Step 4 with the full task.',
    '- **split (max 5 sub-modules):** create sub-runs under `sub-runs/<sub-id>/`, dispatch N parallel sub-orchestrator subagents (each runs its own full loop), then run the assembly loop (Step 5).',
    '',
    '## Step 4 — Generator → Evaluator loop',
    '',
    'Repeat until a stop condition fires:',
    '',
    '1. **Generate:** Dispatch a Generator subagent (payload from `prompts/generator.md`) using the selected adapter, Planner output, `best.md` as baseline, merged evaluator feedback, and the most recent regression note (if any). Write output to `artifacts/round-<N>.md`. If N > 1, delete `artifacts/round-<N-1>.md`.',
    ...
    '',
    '## Step 5 — Assembly loop (only when decomposition occurred)',
    '',
    '1. Concatenate or merge sub-loop `best.md` outputs in order → `assembly/round-1.md`.',
    '2. Run the Generator → Evaluator loop using `rubrics/assembly.yaml` (threshold 85, max_rounds 3, no_improve_rounds 1).',
    '3. Assembler subagent (payload from `prompts/assembler.md`) may only make adapter-allowed consistency changes. For code plan/patch mode, it merges plans and patch proposals. For direct implementation mode, it must not claim tests passed without command output.',
    ...
    '## Final output',
    '',
    'Emit `runs/<run-id>/artifacts/best.md` (or the adapter-specific best artifact) and `runs/<run-id>/scores/summary.json`.',
...
export function getDefaultRalphLoopGeneratorPrompt(): string {
  return [
    ...
    '',
    'You are a Generator subagent in a generic artifact refinement workflow. Your sole job is to produce the next improved artifact candidate for the selected adapter.',
    '',
    '---',
    '',
    '[ADAPTER]',
    ...
    '',
    '[INSTRUCTION]',
    'Based on the Current Best and Planner Output above, make targeted improvements that address the Evaluator Feedback.',
    'Do not rewrite from scratch unless the task-spec explicitly calls for a first draft.',
    'Do not expand the scope beyond what the task-spec defines.',
    '',
    'Adapter-specific rules:',
    '- document: output the full improved Markdown document.',
    '- code + plan/patch mode: output a complete implementation plan, affected files, patch proposal or diff guidance, test strategy, risk notes, and blockers. Do not claim files were edited.',
    '- code + direct implementation mode: output the exact files changed, behavioral summary, tests run with command output, and remaining risks. Do not invent test results.',
    '- decision: output the full revised decision artifact with recommendation, tradeoffs, risks, and assumptions.',
    '',
    'Output only the full artifact candidate in Markdown.',
    '',
  ].join('\n');
}
...
export function getDefaultRalphLoopEvaluatorPrompt(): string {
  return [
    ...
    '{{persona_prompt}}',
    '',
    'You are evaluating an artifact candidate on the specific criteria assigned to you. Score each criterion independently on a 0-100 scale using the rubric text provided.',
    '',
    '---',
    '',
    '[ADAPTER]',
    ...
    '- reasoning must reference specific text in the artifact.',
    '- Each suggestions item must be a concrete, actionable change (not "improve clarity").',
    '- For code tasks, do not claim tests pass unless the artifact includes actual command output.',
    '',
  ].join('\n');
}
...
export function getDefaultRalphLoopAssemblerPrompt(): string {
  return [
    ...
    '',
    'You are an Assembler subagent. You receive independently-refined artifacts from split Ralph Loop sub-runs and merge them according to the selected adapter.',
    '',
    '---',
    '',
    '[ADAPTER]',
    ...
    '',
    '[INSTRUCTION]',
    'Improve only adapter-allowed consistency issues:',
    '- document: transitions, tone uniformity, heading levels, bullet style, and spacing',
    '- code + plan/patch mode: merge implementation plans, patch proposals, file ownership notes, test strategy, and risks without claiming edits happened',
    '- code + direct implementation mode: merge summaries and surface conflicts; do not alter code claims or invent verification',
    '- decision: merge recommendations, tradeoffs, assumptions, and risks into one coherent decision artifact',
    '',
    '**Strictly forbidden:**',
    '- Altering any claim, fact, figure, evidence, or test result without source support',
    '- Reordering sections or work items unless the adapter instructions permit it',
    '- Adding new implementation claims',
    '- Removing blockers or risks',
    '',
    'If you identify content-level issues, contradictions, factual conflicts, file ownership conflicts, or missing verification, do not hide them.',
    'Append a separate "Integration Issues Report" section at the end of your output, listing each issue with its location.',
    '',
    'Output the full revised artifact in Markdown, followed by the Integration Issues Report (if any).',
    '',
  ].join('\n');
}
...
export function getDefaultRalphLoopCodeImplementationRubric(): string {
  return [
    ...
    '      60-74: works in concept but bends local patterns or leaves API boundaries unclear',
...
export function getDefaultRalphLoopDecomposeHeuristics(): string {
  return [
    ...
    '| Condition | Score |',
    '|---|---|',
    '| Estimated artifact > ~800 words, or code task touches 3+ files/modules | +1 |',
    '| task-spec contains an explicit list of sub-sections, work items, or file/module ownership areas | +2 |',
    '| Rubric includes a `completeness`-style or `integration_fit`-style criterion mentioning multiple independent parts | +1 |',
    ...
    '',
    'When splitting, the Orchestrator derives per-unit rubrics from the main rubric:',
    '- Criteria whose scope is unit-local quality (e.g., document section quality, file-local correctness, maintainability) → included in sub-rubrics',
    '- Criteria whose scope is whole-artifact concern (e.g., audience fit, integration fit, overall length, cross-module risk) → dropped from sub-rubrics and evaluated only in the assembly rubric',
    '',
...
export function getDefaultRalphLoopTaskSpecTemplate(): string {
  return [
    ...
    '## Subject',
    '',
    '<!-- One sentence: what artifact are we producing or improving? -->',
    '',
    '<subject>',
    '',
    '## Task type',
    '',
    '<!-- document | code | decision | unknown -->',
    '',
    '<task-type>',
    '',
    '## Selected adapter',
    '',
    '<!-- document | code | decision | user-defined -->',
    '',
    '<adapter>',
    '',
    '## Execution mode',
    '',
    '<!-- document | plan/patch mode | direct implementation mode | decision -->',
    '',
    '<execution-mode>',
    '',
    '## Success criteria',
    '',
    '<!-- What must be true for the artifact to be good enough? -->',
    '',
    '- <criterion>',
    '',
    '## Constraints',
    '',
    '<!-- Scope, style, compatibility, format, files, things to avoid. -->',
    '',
    '- <constraint>',
    '',
    '## Affected files or areas',
    '',
    '<!-- Especially important for code tasks. Use "none" for non-code tasks. -->',
    '',
    '- <path-or-area>',
    '',
    '## Verification expectations',
    '',
    '<!-- Tests, build commands, review criteria, or evidence required. -->',
    '',
    '- <verification>',
    '',
    '## Context',
    '',
    '<!-- Background material, existing drafts, design notes, links, or code references. -->',
    '',
    '<optional>',
    '',
  ].join('\n');
